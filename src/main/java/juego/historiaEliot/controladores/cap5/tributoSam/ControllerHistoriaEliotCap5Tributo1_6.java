...
public class ControllerHistoriaEliotCap5Tributo1_6 {
    ...
    public void initialize() {
        ...
        aliado = (Aliado) new PersonajeDAO().cargarPersonajePorNombre("RayAliado");
    ...
    private void ejecutarAtaque(int index) {
        ...
        PauseTransition esperaAliado = new PauseTransition(Duration.seconds(1.5));
        esperaAliado.setOnFinished(e -> {
            boolean hizoAlgo = combate.atacarConAliado(mensajeCombate);
            if (hizoAlgo) actualizarVida();
            ...
            PauseTransition espera = new PauseTransition(Duration.seconds(3));
            espera.setOnFinished(x -> ejecutarTurnoEnemigo());
            espera.play();
        });
        esperaAliado.play();
    }
    ...
    private void ejecutarTurnoEnemigo() {
        ...
        esperaUso.setOnFinished(e1 -> {
            boolean redirigir = combate.redirigirAtaqueAlAliado();
            Personaje objetivo = redirigir ? combate.getAliado() : jugador;

            Ataque ataque = enemigo.elegirAtaque();
            boolean impacto = combate.aplicarAtaque(enemigo, objetivo, ataque);
            ...
            if (combate.getAliado() != null) combate.getAliado().avanzarTurno();
    ...
    private void reiniciarCombate() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/historiaEliot/Cap5/tributoSam/historiaEliotCap5Tributo1-6.fxml"));
    ...
    private void irASiguienteVista() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/historiaEliot/Cap5/tributoSam/historiaEliotCap5Tributo1-7.fxml"));
            Parent root = loader.load();
            Stage stage = (Stage) mensajeCombate.getScene().getWindow();
            stage.setScene(new Scene(root));
            stage.show();
