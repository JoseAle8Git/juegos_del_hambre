...
public class ControllerHistoriaEliotCap1_0 implements Initializable {
    ...
    private void irAHistoriaEliotCap1_1() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/historiaEliot/Cap1/historiaEliotCap1-1.fxml"));
            Parent root = loader.load();
            Stage stage = (Stage) fraseLabel.getScene().getWindow();
            stage.setScene(new Scene(root, 1221, 720));
        } catch(Exception ex){
            ex.printStackTrace();
            mostrarAlerta("Error", "No se pudo mostrar la siguiente vista historiaEliotCap1-1");
        }
    }
